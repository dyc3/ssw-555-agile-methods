...
Agile and Open Source management styles are quite similar in a few different aspects. Regarding the process of developing software, while agile methods have predefined processes and sequences, numerous open source projects also have structured processes. @Eckstein_2004 Agile is built on individual capability and the motivation of those individuals, each person equally invested and contributing to the project. Open source differs from this in that there are a large number individuals participating in development, but a few Chief Programmers who contribute the majority of the source code. 

The way agile and open source teams function are quite similar. Self-organization is a shared trait among agile and open source teams. In Agile, self-organized teams are important to adapt and adjust to changing requirements. In open source, there is a common focus so all contributing individuals join and follow the goals, organizing in a way that results in maxium productivity. @Eckstein_2004 Close collaboration is required within the team (in methods of pair programming, review, etc), whereas open source teams are scattered in different parts of the world, working asyncronously. This makes the nature of collaborative development different but effective for the tasks at hand.  
